...
export default class MovieItemRoute extends Route {
  async model(params) {
    const { movie_id } = params;
    let response = await fetch(
      `https://api.themoviedb.org/3/movie/${movie_id}?api_key=5136e1d0babe7ebe6c75976bec09bac4&language=en-US`
    );
    let data = await response.json();

    let response1 = await fetch(
      `https://api.themoviedb.org/3/movie/${movie_id}/images?api_key=5136e1d0babe7ebe6c75976bec09bac4`
    );
    let data1 = await response1.json();

    let response2 = await fetch(
      `https://api.themoviedb.org/3/movie/${movie_id}/credits?api_key=5136e1d0babe7ebe6c75976bec09bac4`
    );
    let data2 = await response2.json();

    let response3 = await fetch(
      `https://api.themoviedb.org/3/movie/${movie_id}/videos?api_key=5136e1d0babe7ebe6c75976bec09bac4&language=en-US`
    );
    ...
    if (data1.backdrops.length > 0) {
      ...
      if (backdrop.length === 1) {
        ...
        oribackdrop =
          'https://image.tmdb.org/t/p/original' + backdrop[1].file_path;
      }
    }
    ...
    let director = data2.crew.filter((crew) => crew.job === 'Director');
